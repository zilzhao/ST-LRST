...
if __name__ == '__main__':
    file_name = 'Spatial_syn_mat_#1.csv'
    ...
    np.savetxt('Ano_mat_LRST_#1.csv', tl.unfold(S, mode=0), fmt='%.2f', delimiter=',')

    # Accuracy assessment
    filename3 = 'Spatial_anomaly_mat_#1.csv'
    ano_mat = np.array(pd.read_csv(filename3, header=None))
    # Anomaly information
    filename4 = 'Spatial_anomaly_infor_#1.csv'
    ...
    np.savetxt('Acc_mat_LRST_#1.csv', acc_mat, fmt='%.3f', delimiter=',')
